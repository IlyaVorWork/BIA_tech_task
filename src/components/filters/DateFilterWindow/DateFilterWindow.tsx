...
const DateFilterWindow: FunctionComponent = () => {
  ...
  const [daysOffset, setDaysOffset] = useState<number>(
    new Date(currentYear, currentMonth, 0).getDay() > 0
      ? new Date(currentYear, currentMonth, 0).getDay()
      : 6,
  );
    ...
    >
      <Stack direction={"row"} justifyContent={"space-between"}>
        <Stack direction={"row"} gap={"0.25rem"} alignItems={"center"}>
          <MdChevronLeft
            className={`${styles.dateFilterChevronIcon} ${currentMonth == 0 ? styles.disabled : ""}`}
            fontSize={"1.25rem"}
            onClick={() => {
              if (currentMonth != 0) setCurrentMonth((prev) => prev - 1);
            }}
          />
          <Text fontWeight={"500"} width={"4.75rem"} textAlign={"center"}>
            {months[currentMonth]}
          </Text>
          <MdChevronRight
            className={`${styles.dateFilterChevronIcon} ${
              currentMonth == 11 ||
        ...
        <Stack direction={"row"} gap={"0.25rem"} alignItems={"center"}>
          ...
          />
          <Text fontWeight={"500"} width={"2.5rem"} textAlign={"center"}>
            {currentYear}
          </Text>
          <MdChevronRight
            className={`${styles.dateFilterChevronIcon} ${currentYear === new Date().getFullYear() ? styles.disabled : ""}`}
            onClick={() => {
      ...
      >
        <For each={daysOfWeek}>
          {(day) => (
            <Center fontSize={"0.875rem"} width={"2rem"}>
              {day.slice(0, 1)}
            </Center>
          )}
        </For>
